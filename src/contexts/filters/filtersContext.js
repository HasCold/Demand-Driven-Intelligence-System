...
const FiltersProvider = ({ children }) => {
    ...
    const { products: sourceProducts, loading: productsLoading, error: productsError } = useContext(productsContext);


    /* Loading products from API into filter state */
    useEffect(() => {
        ...
        const dynamicCategoryMenu = uniqueCategories.map((label, i) => ({
            id: i + 1,
            label,
            checked: false,
        }));
        ...
        const dynamicBrandsMenu = uniqueBrands.map((label, i) => ({
            id: i + 1,
            label,
            checked: false,
        }));
        ...
        dispatch({
            ...
            payload: {
                ...
                minPrice,
                maxPrice,
                brandsMenu: dynamicBrandsMenu.length ? dynamicBrandsMenu : brandsMenu,
                categoryMenu: dynamicCategoryMenu.length ? dynamicCategoryMenu : categoryMenu,
            }
        });

    }, [sourceProducts, productsLoading]);


    /* function for applying Filters - (sorting & filtering) */
    const applyFilters = () => {

        let updatedProducts = [...sourceProducts];
    ...
    }, [state.sortedValue, state.updatedBrandsMenu, state.updatedCategoryMenu, state.selectedPrice, sourceProducts, productsLoading]);
